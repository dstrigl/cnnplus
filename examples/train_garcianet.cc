/**************************************************************************//**
 *
 * \file   train_garcianet.cc
 * \author Daniel Strigl, Klaus Kofler
 * \date   Jul 07 2009
 *
 * $Id: train_garcianet.cc 1724 2009-07-12 20:26:31Z dast $
 *
 *****************************************************************************/
...
{
    if (argc < 3) {
        printf("Usage: %s <faces-train-file> <faces-test-file>\n", argv[0]);
        return -1;
    }
    ...
    try {
        // Load the CBCL face datasets
        CbclFaceDataSrc<DataType> facesTrn(Size(20, 20));
        CbclFaceDataSrc<DataType> facesTst(Size(20, 20));
        facesTrn.load(argv[1]);
        facesTst.load(argv[2]);

        // Initialize the network
        GarciaNet<DataType> net;
        net.forget();
