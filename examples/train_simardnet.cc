/**************************************************************************//**
 *
 * \file   train_simardnet.cc
 * \author Daniel Strigl, Klaus Kofler
 * \date   Jun 17 2009
 *
 * $Id: train_simardnet.cc 3561 2010-11-22 21:14:16Z klaus $
 *
 *****************************************************************************/
...
#if 0   // Create new displacement fields for the distorted data
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        SubsetDataSrc<DataType> mnistTrn(mnistTrain,     0, 54);
        SubsetDataSrc<DataType> mnistVal(mnistTrain, 54000,  60);
...
#endif
#if 0   // Create additional elastic distorted data
        ElastDistDataSrc<DataType> dstMnistTrn(mnistTrn, 9, 22, 4);
#endif
        // Initialize the network
        CuSimardNet<DataType> net;
        net.forget(0.05f);

        // Start training
        MyAdapter<DataType, ErrFnc> adapter(net, mnistVal, mnistTst, argc >= 6 ? argv[5] : "train.py");
        OnlineTrainer<DataType, ErrFnc> trainer(net, 0.005f, adapter);
