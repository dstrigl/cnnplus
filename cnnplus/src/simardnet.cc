 ...
 * \date   Jan 03 2009
 *
 * $Id: simardnet.cc 1942 2009-08-05 18:25:28Z dast $
 *
 * \brief  Implementation of cnnplus::SimardNet.
 *
 *****************************************************************************/
...
template<typename T>
SimardNet<T>::SimardNet(Size const & sizeImgIn,
...
SimardNet<T>::forget(T const sigma, bool scale)
{
    matvecli::randreseed();
...
template<typename T> void
SimardNet<T>::update(T const eta)
{
    layer1_.update(eta);
    layer2_.update(eta);
    layer3_.update(eta);
...
template<typename T> void
SimardNet<T>::fprop(T const * in, T * out)
{
...
template<typename T> void
SimardNet<T>::bprop(T * in, T const * out, bool accumGradients)
{
...
template<typename T> void
SimardNet<T>::load(std::string const & filename)
{
#ifdef CNNPLUS_MATLAB_FOUND
...
template<typename T> void
SimardNet<T>::save(std::string const & filename) const
{
#ifdef CNNPLUS_MATLAB_FOUND
...
template<typename T> void
SimardNet<T>::writeOut(std::string const & filename, T * const out) const
{
#ifdef CNNPLUS_MATLAB_FOUND
    MATFile * file = matOpen(filename.c_str(), "w");
    if (!file)
...
template<typename T> void
SimardNet<T>::trainableParam(typename NeuralNet<T>::TrainableParam & param)
{
    param.resize(4);
...
SimardNet<T>::toString() const
{
    std::stringstream ss;
    ss << "SimardNet<" << typeid(T).name() << ">"
       << "[L1:" << layer1_.toString()
       << ",L2:" << layer2_.toString()
       << ",L3:" << layer3_.toString()
       << ",L4:" << layer4_.toString()
...
SimardNet<T>::numTrainableParam() const
{
...
template<typename T> size_t
SimardNet<T>::numConnections() const
{
...
/*! \addtogroup eti_grp Explicit Template Instantiation
 @{
 */
template class SimardNet<float>;
template class SimardNet<double>;
/*! @} */
